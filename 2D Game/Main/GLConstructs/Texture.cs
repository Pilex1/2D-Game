...
namespace Game.Main.GLConstructs {
    //modified version from giawa's opengl4csharp library
    public class Texture : IDisposable {
        ...
        ~Texture() {
            if (TextureID != 0)
                Debug.Fail("Texture not dispoed: " + Filename);
        }
        #endregion
        ...
        private void LoadBitmap(Bitmap BitmapImage, bool FlipY = true) {
            ...
            GL.TexImage2D(TextureTarget, 0, PixelInternalFormat.Rgba8, BitmapImage.Width, BitmapImage.Height, 0, Pencil.Gaming.Graphics.PixelFormat.Bgra, PixelType.UnsignedByte, bitmapData.Scan0);

            GL.TexParameterI(TextureTarget, TextureParameterName.TextureMagFilter, new int[] { (int)TextureMagFilter.Nearest });
            GL.TexParameterI(TextureTarget, TextureParameterName.TextureMinFilter, new int[] { (int)TextureMinFilter.Nearest });//(int)TextureParam.Linear);   // linear filter
        ...
        private void LoadDDS(string ResourceFile) {
            using (BinaryReader stream = new BinaryReader(new FileStream(ResourceFile, FileMode.Open))) {
                ...
                try {
                    ...
                    GL.TexParameterI(TextureTarget, TextureParameterName.TextureMinFilter, new int[] { (int)TextureMagFilter.Linear });
                    ...
                    for (int i = 0; i < (imageData.MipmapCount == 0 ? 1 : imageData.MipmapCount); ++i) {
                        ...
                        if (compressed) {
                            ...
                            GL.TexImage2D(TextureTarget, i, format, nWidth, nHeight, 0, Pencil.Gaming.Graphics.PixelFormat.Bgra, PixelType.UnsignedShort4444, (IntPtr)(pinned.AddrOfPinnedObject().ToInt64() + nOffset));
                        }
