...
namespace Game.Terrains {
    ...
    internal static class TerrainGen {
                    ...
                    for (int j = 0; j < WidthFactor; j++) {
                        ...
                        for (int k = 0; k <= y; k++) {
                            if (k >= y - (6 + MathUtil.RandInt(Rand, 0, 3))) {
                                SetTile(x, k, Tile.Snow);
                            } else if (k >= 12 + MathUtil.RandInt(Rand, 0, 3)) {
                                SetTile(x, k, Tile.Dirt);
                            } else {
                                SetTile(x, k, Tile.Stone);
                            }
                        }
        ...
        private static void SetTile(int x, int y, Tile id, bool overwrite = false) {
