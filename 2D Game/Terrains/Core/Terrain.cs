...
namespace Game.Terrains {
	...
	static class Terrain {
		...
		public static void LoadChunk(ChunkData data) {
			...
			LightingManager.Instance.LoadLightings (data.location, data.lightings);
		...
		private static ChunkData CopyRegion(int region) {
			...
			for (int i = 0; i < TerrainGen.ChunkSize; i++) {
				...
				for (int j = 0; j < Tiles.GetLength (1); j++) {
					tiles [i, j] = Tiles [x, j];
					lightings [i, j] = LightingManager.Instance.GetLighting (x, j);
				}
			}
		...
		public static bool HasNeighbouringSolidTiles(int x, int y) {
			...
			if (l != null && l.tileattribs.solid)
				return true;
			if (r != null && r.tileattribs.solid)
				return true;
			if (d != null && d.tileattribs.solid)
				return true;
			if (u != null && u.tileattribs.solid)
				return true;
			if (m != null && m.tileattribs.solid)
				return true;
			return false;
		}
		...
		private static bool Valid(int x, int y, int height) {
			...
			for (int i = 0; i < height; i++) {
				if (y + i >= Tiles.GetLength (1))
					continue;
				if (Tiles [x, y + i].enumId != TileID.Air)
					return false;
			}
			return true;
		}
		...
		internal static int HighestPoint(int x) {
			...
			for (int i = Tiles.GetLength (1) - 1; i > 0; i--) {
				if (Tiles [x, i].enumId != TileID.Air)
					return i + 1;
			}
			return 1;
		}
		...
		public static Tile TileAt(Vector2i v) {
		...
		public static Tile BreakTile(int x, int y) {
			...
			if (tile.enumId == TileID.Air)
				return Tile.Air;
			if (tile.enumId == TileID.Bedrock)
				return Tile.Invalid;
